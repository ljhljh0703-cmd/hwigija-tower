...
**마지막 갱신 2026-09-02** · 미완 **2건** (M-2 push · M-3 청소). M-1 은 닫혔다

---

## ✅ M-1. Unity 라이선스 — **작가 「실행 완」 (2026-09-02). 확인은 코덱스가 한다**

작가가 Unity CLI 를 손봤다고 보고했다. **내가 직접 확인하지는 못했다** — 라이선스 클라이언트는
연결 폴더 밖이라 내 사정권 밖이다. 그래서 **다시 물어보지 않는다.**
코덱스가 M3 착수 전 `Tools/unity_license_probe.sh` 를 먼저 돌리게 해뒀다.
**아직 FAIL 이면 코덱스가 로그를 그대로 물고 온다.** 작가가 또 확인해줄 필요 없다.

아래는 **FAIL 이 다시 뜰 때만** 보면 되는 원본 기록이다.

<details>
<summary>원래 증상과 후보 조치 (접어둠)</summary>
