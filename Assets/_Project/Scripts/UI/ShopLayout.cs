...
{
    public static class ShopLayoutRuntime
    {
        public static Vector2 AnchorMin(ShopSlot slot) => new Vector2(slot.X, 1f - slot.Y - slot.Height);
        public static Vector2 AnchorMax(ShopSlot slot) => new Vector2(slot.X + slot.Width, 1f - slot.Y);
    }

    public static class ShopUiTokens
    {
        ...
        private static Color Parse(string html)
        {
            ...
            {
                throw new InvalidOperationException("Invalid shop UI token: " + html);
            }
