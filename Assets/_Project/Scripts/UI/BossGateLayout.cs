...
{
    public static class BossGateLayoutRuntime
    {
        public static Vector2 AnchorMin(BossGateSlot slot) => new Vector2(slot.X, 1f - slot.Y - slot.Height);
        public static Vector2 AnchorMax(BossGateSlot slot) => new Vector2(slot.X + slot.Width, 1f - slot.Y);
    }

    public static class BossGateUiTokens
    {
        ...
        private static Color Parse(string html)
        {
            ...
            {
                throw new InvalidOperationException("Invalid boss gate UI token: " + html);
            }
