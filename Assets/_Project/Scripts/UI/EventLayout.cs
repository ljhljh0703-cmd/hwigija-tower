...
{
    public static class EventLayoutRuntime
    {
        public static Vector2 AnchorMin(EventSlot slot) => new Vector2(slot.X, 1f - slot.Y - slot.Height);
        public static Vector2 AnchorMax(EventSlot slot) => new Vector2(slot.X + slot.Width, 1f - slot.Y);
    }

    public static class EventUiTokens
    {
        ...
        private static Color Parse(string html)
        {
            ...
            {
                throw new InvalidOperationException("Invalid event UI token: " + html);
            }
