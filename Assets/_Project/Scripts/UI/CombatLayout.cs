...
{
    public static class CombatLayoutRuntime
    {
        public static Vector2 AnchorMin(CombatSlot slot)
        {
            return new Vector2(slot.X, 1f - slot.Y - slot.Height);
        }

        public static Vector2 AnchorMax(CombatSlot slot)
        {
            return new Vector2(slot.X + slot.Width, 1f - slot.Y);
        }
    }

    public static class CombatUiTokens
    {
        ...
        private static Color Parse(string html)
        {
            ...
            {
                throw new InvalidOperationException("Invalid combat UI token: " + html);
            }
